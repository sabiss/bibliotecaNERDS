    ...
    for (let livro of livros) {
      ...
      tbody.innerHTML += `
                <tr>
                    ...
                    <td onclick="editar('${livro._id}')"><div class="editar iconesAcao"><img src="../../assets/lapisEdicao.png"></div></td>
...
async function buscarLivros() {
  ...
  try {
    const respostaApi = await fetch(
      `http://localhost:3000/buscarLivros?palavra=${filtro}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    for (let livro of livros) {
      ...
      livrosComQuantidadeDeCopias.push({
        livro,
        numeroDeCopias: copias.length,
      });
    }
...
async function remover(idLivro) {
  try {
    const respostaApi = await fetch(
      `http://localhost/deletarLivro/${idLivro}`,
      {
        method: "DELETE",
        mode: "cors",
        headers: {
